    ...
    <div class="projects-grid">
      ${h.map((e,a)=>`
        <div class="card reveal reveal-delay-${a%4+1}">
          ...
          <a href="${e.title.includes("Paniculas")?"#proyecto/paniculas":e.title.includes("Cloud")?"#proyecto/cloud":e.title.includes("DSS")?"#proyecto/dss":"#"}" class="card-link">
            Ver detalles
            ${i.arrow}
          </a>
        </div>
...
`,v=()=>{const e=window.location.hash,a=document.querySelector("header");e==="#proyecto/paniculas"?(c.innerHTML=`
    ...
    `,a.style.display="none"):(c.innerHTML=`
      ...
      ${l()}
      ${j()}
    `,a.style.display="block",E(),L()),P(),window.scrollTo({top:0,behavior:"smooth"})};window.addEventListener("hashchange",v);document.addEventListener("DOMContentLoaded",()=>{v()});
